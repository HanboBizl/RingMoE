...
from ringmoe_framework.datasets.transforms.aug_policy import ImageNetPolicyV2
from ringmoe_framework.datasets.transforms.mixup import Mixup
from ringmoe_framework.datasets.transforms.random_erasing import RandomErasing
from ringmoe_framework.datasets.utils import _check_finetune_dataset_config
...
def build_transforms(config, interpolation, is_train=True):
    ...
    if is_train:
        ...
        trans += [P.RandomColorAdjust(brightness=config.color_jitter,contrast=config.color_jitter,saturation=config.color_jitter)]
        ...
        if config.arch == "simmim" or config.arch == "simmim_single_moe":
