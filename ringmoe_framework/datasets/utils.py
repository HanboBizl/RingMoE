...
def _check_pretrain_dataset_config(config: dict):
    ...
    config.pretrain_dataset.epoch = config.train_config.epoch

    if config.arch == "simmim" or config.arch == "ringmo_framework" or config.arch == "ringmo_mm" or config.arch == "simmim_moe" \
            or config.arch == "simmim_single_moe":
        config.pretrain_dataset.mask_patch_size = config.model.mask_patch_size

    if config.train_config.batch_size:
        if config.parallel.parallel_mode == "semi_auto_parallel":
            config.pretrain_dataset.batch_size = config.train_config.batch_size*config.parallel_config.data_parallel * config.parallel_config.micro_batch_num
        else:
